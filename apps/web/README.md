...
**Promo art sizing:** on desktop (3-column grid) the right column is a **tall narrow strip** (~**150–190px** wide × ~**260–380px** tall in CSS, height grows with text). Klarna uses **`object-fit: cover`** (fills the strip; edges may crop) — prefer **portrait ~9:16 or 2:3**. **Public** uses **`object-fit: contain`** on a dark column background so a **wide landscape** banner stays **fully readable** (letterboxing OK). **Robinhood** uses **`object-fit: contain`** on a **lime** column background (`#dafc51`, matched to `robinhood-promo.png`) so letterboxing blends with the asset and the **wide landscape** wordmark stays **fully visible** (same idea as Public). If `imageSrc` is omitted, the circular green fallback is shown. On mobile the block is **full card width** with **min-height ~200px**.
...
- `/categories/[slug]` — stores in that category with offers (categories: Auto, Electronics, Entertainment, Finance, Food, Shopping, Sports, Travel — **A–Z** in the carousel and Stores mega menu via `getCategories()` / `getStoresMegaMenu()`)
